...
const RULES = [
  ...
  { id: "C-2", votes: "32 / 32 узла с тенью", title: "Тень есть только у стрелок карусели и всплывающих слоёв.", text: "Все 32 узла с тенью — <code>swiper-button-shadow</code>.", exception: "В корпусе нарушителей нет.", pages: ["education-center"] },
  { id: "C-3a", votes: "223 / 232", title: "Кнопка и поле высотой 40–56 — радиус 12.", exception: "Поля формы поиска — 0: стыкуются в общей обёртке; у самих <code>input</code> своего края нет.", specimen: [["courses-listing", "div.grid.grid-cols-4.gap-3.pb-4 button.rounded-xl", 0], ["education-centers-listing", "section a.inline-flex.w-full.rounded-xl", 0]], pages: ["courses-listing", "education-centers-listing"] },
  ...
  { id: "C-3c", votes: "138 / 138 · 267 / 268", title: "Фото человека — круг; логотип организации — квадрат со скруглением в четверть стороны.", text: "24 → 6, 32 → 8, 48 → 12, 100 → 24. Знаменатель — все 299 квадратов замеров минус 31 без своего радиуса: их углы режет предок.", exception: "Значок школы на аватаре профиля: 40 → 14.", specimen: [["courses-listing", "div.flex.items-center.gap-2.whitespace-nowrap:has(> img.rounded-lg)", 0], ["authors", "div.relative.h-\\[68px\\].w-\\[68px\\]", 0]], pages: ["courses-listing", "authors", "author"] },
  ...
  { id: "C-6", votes: "37 / 37", title: "Строка таблицы и номер листания — фон <code>bg-ui-black-50</code>.", exception: "В корпусе нарушителей нет. Класс <code>phone:hover:bg-none</code> у строк таблицы скомпилирован, но убирает он картинку фона, а фон строке даёт <code>background-color</code>: до 767 фон остаётся.", pages: ["rating", "courses-listing"] },
  { group: "Адаптив" },
  ...
  { id: "CT-2b", votes: "23 / 23", title: "Счётчик пишется слитно: «1419 отзывов», «+18070».", exception: "В корпусе нарушителей нет. Ещё две слитные подписи от тысячи — номер лицензии школы, это не счётчик; телефон «8 800 700-93-29» пишется с пробелами, но это не разряды.", pages: ["education-centers-listing", "education-center"] },
