...
| `ad-card` | `hover` (ослаблено) | **снято — слабый факт** | корень `div.cursor-pointer`, внутри два `<img>`, у 20 узлов кнопка «ещё» 24×24 (символ `more`; до пересмотра 11 сентября 2026 называлась здесь `IconButton` закрытия), у 4 — ссылка `a.absolute` на рекламодателя; на корне нет ни `<a>`/`<button>`, ни `hover:*` |
...
Итог по реестру после дополнения — **187 пар** «запись × состояние»:
148 сняты источником, 2 дописаны нормативом, 37 не сняты и адресованы
поимённо. Числа §5 описывают 55 записей на момент R1-02 и не
переписываются: прибавка — ровно 40 пар, 17 из них `default`; пересмотр §8 перевёл часть остальных в снятые.
