...
| **Живая реализация** | [`viewer/index.html#checkbox`](../../../viewer/index.html#checkbox) |
...
**Правило.** Двоичный выбор с подписью: отмечено или нет. В разметке снятых страниц не встречается, но продуктовый CSS-чанк `similar-courses` несёт его стили — отмеченное, отключённое и фокус; story `BaseCheckbox` рисует один чекбокс с подписью.

## Когда не использовать

- Для фильтрации выдачи на месте: на снятых страницах направление выбирают `FilterChip`, чекбоксов на них нет.

## Как работает

Скрытый нативный `input` и видимая кнопка `.base-checkbox__button`; отметку рисует правило `.base-checkbox__input:checked+.base-checkbox__button`.

## Управление клавиатурой

Нативный чекбокс: фокус по Tab, переключение — Space.

## Анимация

Переходов нет ни в чанке `similar-courses`, ни в CSS story: отметка появляется мгновенно.
...
Адаптивных правил у чекбокса нет.
