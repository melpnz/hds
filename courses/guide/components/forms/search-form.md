...
| **Живая реализация** | [`viewer/index.html#search-form`](../../../viewer/index.html#search-form) |
...
Корень (`<div>`) в фокус не попадает. На один экземпляр по Tab проходят: 1 кнопка и 3 поля `input type=text` — у 4 из 5; 1 кнопка и 2 поля `input type=text` — у 1 из 5. По корпусу (33 узла): `Button` — 5, внутри `TextInput` — 28. Фокус внутреннего поля показывает рамка обёртки `focus-within:border-ui-black-850`. 14 из 33 фокусируемых узлов на 1440 скрыты и в порядок табуляции не входят, пока их не покажут.

## Анимация

Переходы вычислены по страницам, отрисованным с CSS корпуса на 1440:

- `transform` за 0,15 с, кривая `cubic-bezier(0.4, 0, 0.2, 1)`: `svg.svg-icon.mr-1.shrink-0` — 28 узлов, у 5 экземпляров из 5.
...
Классы с префиксом ширины в поддереве записи: `phone:hidden`, `phone:grid`, `phone:grid-cols-[1fr]`, `phone:gap-4`, `phone:pb-6`, `phone:pt-6`. Условия префиксов: `phone:` — до 767 (`docs/guide/layout.md`).
