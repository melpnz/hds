...
| `readOnly` | принято владельцем | Оболочка как у Disabled, текст внутри как у Default; используется нативный атрибут `readonly`. |
...
- **ReadOnly.** Зафиксирован review-квизом 18 сентября 2026: серая недоступная оболочка, но обычный контраст введённого текста.
