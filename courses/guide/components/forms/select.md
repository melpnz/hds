...
| **Живая реализация** | [`viewer/index.html#select`](../../../viewer/index.html#select) |
...
**Правило.** Используйте для выбора значения в форме поиска — города, организации, типа. В продукте 28 узлов на 5 страницах, все внутри `SearchForm` в hero. Половина из них, 14, — мобильный дубль формы (`div.hidden … phone:grid`): на 1440 он скрыт.
...
Триггер — поле `TextInput` внутри `v-popper`, справа шеврон. Открытый список гидрируется и в снятой разметке отсутствует; поворачивается ли шеврон при открытии, не видно: `.rotate-180` в CSS корпуса объявлен, но в снятой разметке его нет ни на одном узле.

## Управление клавиатурой

Корень (`<div>`) в фокус не попадает. На один экземпляр по Tab проходят: 1 поле `input type=text` — у 14 из 28; ничего — у 14 из 28. По корпусу (28 узлов): внутри `TextInput` — 28. Фокус внутреннего поля показывает рамка обёртки `focus-within:border-ui-black-850`. 14 из 28 экземпляров на 1440 скрыты (`display:none` у предка) и в порядок табуляции не входят.

## Анимация

Переходы вычислены по страницам, отрисованным с CSS корпуса на 1440:

- `transform` за 0,15 с, кривая `cubic-bezier(0.4, 0, 0.2, 1)`: `svg.svg-icon.mr-1.shrink-0` — 28 узлов, у 28 экземпляров из 28.
