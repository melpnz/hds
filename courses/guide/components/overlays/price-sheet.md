...
`PriceSheet` применяется только в мобильном ряду быстрых фильтров. Эталонный контекст — экран 320×568 из Education Figma, общий canvas `9094:48333`; открытая панель — node `9356:52508` размером `320×176`. На tablet и desktop этот bottom sheet не масштабируется: цена открывается как dropdown/popover рядом с trigger.
