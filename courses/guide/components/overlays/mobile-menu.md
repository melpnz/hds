...
| **Живая реализация** | [`viewer/index.html#mobile-menu`](../../../viewer/index.html#mobile-menu) |
...
| сервисы | публичный `ServiceLogo` 24 и подпись через 12 |
...
- **Тексты.** Все пункты — дословно из кадра. В группе «Все сервисы Хабра» — те же четыре проекта и в том же порядке, что у `ServiceLogo` (Хабр, Q&amp;A, Карьера, Курсы), цвета совпали с продуктом.
- **Ассеты.** Крестик и шевроны выводятся через `UIcon` из Tabler, иконки сервисов — через `ServiceLogo`. Своих ассетов у записи нет.
- **Состояния.** Живой пример показывает `closed` как кнопку мобильной шапки и `open` как панель меню. Кнопки открытия и закрытия синхронизируют `aria-expanded`; оба состояния приняты по Figma в review-квизе 18 сентября 2026.
