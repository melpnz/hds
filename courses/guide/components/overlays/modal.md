...
В интерактивном примере переключатель последней оставшейся footer-кнопки блокируется. Это фиксирует обязательное действие и не оставляет пустой footer. Device-вариант выбирается шириной viewer: 320 и 480 показывают mobile bottom sheet, 768 и 1024 — tablet/desktop modal; отдельного переключателя устройства внутри примера нет.
