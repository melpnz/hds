...
- **Состояния.** Живой пример показывает `collapsed`, `expanded`, `hover` и `focus-visible`; кнопка переключает раскрытие и синхронизирует `aria-expanded`. Hover и focus-visible приняты по Figma в review-квизе 18 сентября 2026.
