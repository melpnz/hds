...
| **Живая реализация** | [`viewer/index.html#social-icon`](../../../viewer/index.html#social-icon) |
...
**Правило.** Используйте для ссылок на аккаунты Хабра во внешних сетях. В продукте значок стоит только в подвале: 60 узлов из 60, по шесть на каждой из десяти страниц.

## Когда не использовать

- Для иконок интерфейса — `UIcon` с именем `i-tabler-*`.
- Для профилей людей во внешних сетях — их рисует другой спрайт, `external-profile.svg`, в карточках и шапке профиля экспертов.

## Как работает

Корень — ссылка `a.block.rounded-full` на внешний адрес, внутри — символ спрайта `social-v3.1.svg`. Цвет приходит из самого спрайта, наведение гасит значок до `opacity-80`.

## Управление клавиатурой

Корень — ссылка `<a href>`: фокус по Tab, переход — Enter. Своего кольца фокуса нет, но нет и сброса: работает фокус браузера.
