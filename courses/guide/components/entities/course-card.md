...
| **Живая реализация** | [`viewer/index.html#course-card`](../../../viewer/index.html#course-card) |
...
Ссылка `a.z-2.absolute.inset-0` лежит поверх кнопки: точка в центре «Далее»
попадает в ссылку, и `hover:opacity-90` кнопки не срабатывает — проверено
курсором на собранных страницах (правило C-5 в `docs/guide/composition.md`).
...
Количество видимых тегов зависит от ширины самой карточки, а не только viewport. Живой preview adapter использует container query: на широкой карточке показывает дополнительный тег и пересчитывает `+N`, на узкой оставляет компактный набор. В production этот состав может вычисляться рантаймом из данных.
