...
| **Живая реализация** | [`viewer/index.html#promo-card`](../../../viewer/index.html#promo-card) |
...
**Правило.** Используйте для промокода школы: логотип, плашка кода, условие, кнопка «Открыть код» (на 4 карточках из 20 — «Посмотреть»). В продукте 20 узлов — только на странице промокодов.

## Когда не использовать

- Для рекламы — `AdCard`.
- Для курса со скидкой — `CourseCard` с `Chip tone="orange"`.

## Как работает

`div.cursor-pointer` без сквозной ссылки: единственный фокусируемый узел — кнопка: «Открыть код» у 16 карточек, «Посмотреть» у 4. По макету «Открыть код» открывает `PromoCodeModal`, в продукте модалка гидрируется и не снята.

## Управление клавиатурой

Корень (`<div>`) в фокус не попадает. На один экземпляр по Tab проходят: 1 кнопка — так у всех 20. По корпусу (20 узлов): `Button` — 20.
