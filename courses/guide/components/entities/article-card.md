...
| **Живая реализация** | [`viewer/index.html#article-card`](../../../viewer/index.html#article-card) |
...
**Правило.** Используйте для статьи журнала в ленте: тип и дата («Статья • 28 августа»), заголовок. В продукте 32 узла на 4 страницах, все внутри `Carousel` («Журнал», «Истории успеха»).

## Когда не использовать

- Вне ленты — в снятых страницах карточки статьи в сетке нет.

## Как работает

`div` со сквозной ссылкой `a.z-2.absolute` на статью.

## Управление клавиатурой

Корень (`<div>`) в фокус не попадает. На один экземпляр по Tab проходят: 1 ссылка — так у всех 32. По корпусу (32 узла): `Link` — 32.
