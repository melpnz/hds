...
| **Живая реализация** | [`viewer/index.html#card-grid`](../../../viewer/index.html#card-grid) |
...
### Вариант `school-courses`

На странице школы сетка остаётся вариантом `CardGrid`, но не копией default:
она использует `crs-card-grid--school-courses`, gap 16, четыре колонки на
desktop, три на tablet и одну на phone. На tablet видны первые шесть карточек,
на phone — первые четыре. Источник — production-страница
`/education_centers/35-yandeks-praktikum`; правило закреплено также в
`machine/patterns/education-center.json`.
...
| `empty` | принято по Figma | Пустая выдача с заголовком и подсказкой показана в живом примере без временной маркировки. |
...
- **Empty.** Состояние принято в review-квизе 18 сентября 2026 и входит в обычный контракт компонента.
