...
- **Состояния.** Живой пример показывает `default`, `hover`, `focus-visible`, `pressed`, `disabled`, `loading` и `open`. Прежнее имя `selected` заменено на `pressed` решением review-квиза 18 сентября 2026.
