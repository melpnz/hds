...
| **Живая реализация** | [`viewer/index.html#filter-bar`](../../../viewer/index.html#filter-bar) |
...
`div` с чипами и `overflow-x-auto` без префикса ширины: ряд прокручивается по горизонтали на любой ширине, когда не помещается.

## Управление клавиатурой

Корень (`<div>`) в фокус не попадает. На один экземпляр по Tab проходят: 17 кнопок — у 3 из 5; 10 кнопок — у 1 из 5; 51 кнопка — у 1 из 5. По корпусу (112 узлов): `FilterChip` — 107; вне записей реестра — `button.bg-ui-black-850.text-ui-white.border-ui-black-850.py-2` — 5.
...
Адаптивных классов в разметке нет: запись одинакова на всех ширинах, раскладку меняет контейнер вокруг неё. На узкой ширине прямые дочерние элементы ленты не сжимаются (`flex: 0 0 auto`), поэтому переполнение остаётся внутри `FilterBar` и доступно через горизонтальный скролл, не расширяя страницу.
