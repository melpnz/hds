...
| **Живая реализация** | [`viewer/index.html#link`](../../../viewer/index.html#link) |
...
`<a>`: внутренние адреса — 544, внешние — 305, `mailto:` и `tel:` — 16, якоря — 2; в новой вкладке открываются 224 ссылки. Цвет и подчёркивание при наведении задаёт базовый слой (`a{color:var(--color-links-main)}`, `a:hover{text-decoration:underline}`); общего класса у ссылки нет — корень элементный.

## Управление клавиатурой

Корень — ссылка `<a href>`: фокус по Tab, переход — Enter. Кольцо `focus-visible:outline` стоит у 19 узлов из 867; у остальных своего кольца и сброса нет — работает фокус браузера. 224 из 867 открываются в новой вкладке (`target="_blank"`). 100 из 867 экземпляров на 1440 скрыты (`display:none` у предка) и в порядок табуляции не входят.
