...
`figma-only`: разбивка стала 45 / 4 / 22. Ещё одна запись переехала при сборке страницы `/courses` (R6-01): `pagination` из `figma-only` в `production` — «вместо листания кнопка «Показать ещё 20»» не подтвердилось, листание есть на четырёх листингах. Разбивка — 46 / 4 / 21.
