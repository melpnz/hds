...
`partial` здесь значит ровно то, что сказано в таблице статусов ниже, и стоит
прочесть буквально: разметка снята с продакшена, числа измерены браузером,
правила подняты из корпуса прод-CSS парсером, живой пример стоит на витрине —
а прозаические разделы спецификации (когда не использовать, как работает,
управление клавиатурой, анимация, правила) не написаны, и обязательные
состояния покрыты не все. Каждая такая спецификация называет это про себя
сама, в разделе «Ограничения», и называет шаг, который её дописывает.
...
уже сейчас: 45 элементов сняты с продакшена, 4 есть только в Storybook, 6 —
только в макете.
...
(45 / 4 / 6) сверяются в обоих местах, где написаны, прозой и таблицей.
