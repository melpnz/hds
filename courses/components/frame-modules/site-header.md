...
Корень (`<header>`) в фокус не попадает. С клавиатуры доступны вложенные записи: `Link` ×80, `Button` ×1, `SearchInput` ×1, `TextInput` ×1 — у каждой свой раздел. Фокус внутреннего поля показывает рамка обёртки `focus-within:border-ui-black-850`.
