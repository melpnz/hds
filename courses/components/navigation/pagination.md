...
**Правило.** Используйте для листания выдачи под сеткой карточек — курсов, организаций, промокодов. В продукте пагинация стоит на всех четырёх листингах и только там, по одной на страницу, сразу под `CardGrid`.
...
- **Переклассификация.** До 11 сентября 2026 запись стояла `figma-only` с формулировкой «в проде листания нет: вместо него кнопка «Показать еще 20»». Текста «Показать ещё» нет ни на одной из десяти снятых страниц, а пагинация есть на четырёх. Ошибку нашла сборка страницы `/courses` из записей реестра (шаг R6-01). Вёрстка по макету (корень `crs-pagination`) убрана — её заменила продуктовая разметка.
