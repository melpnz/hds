...
- **Состояния.** Требуемые состояния записи (`default`, `hover`, `focus-visible`, `current`, `disabled`) в макете отдельными вариантами не нарисованы, а измерить их негде. Разбор — [`components/STATE-CAPTURE.md`](../STATE-CAPTURE.md).
...
«pagination», прочитан `get_design_context` 11 сентября 2026.
