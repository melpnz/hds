...
- **Состояния не сняты.** Требуемые состояния записи (`default`, `hover`, `focus-visible`, `current`) в макете отдельными вариантами не нарисованы, а измерить их негде. Разбор — [`components/STATE-CAPTURE.md`](../STATE-CAPTURE.md).
