...
| **Живая реализация** | [`showcase/components.html#c-tab`](../../showcase/components.html#c-tab) |
...
- **Состояния не сняты.** Требуемые состояния записи (`default`, `hover`, `focus-visible`, `selected`, `disabled`) в макете отдельными вариантами не нарисованы, а измерить их негде. Разбор — [`components/STATE-CAPTURE.md`](../STATE-CAPTURE.md).
