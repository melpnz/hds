...
| **Живая реализация** | [`showcase/components.html#c-numbered-course-item`](../../showcase/components.html#c-numbered-course-item) |
...
> и относится к шагу R5-09. Всё, что стоит выше, — измерено.
...
- **Прозаические разделы не написаны.** «Когда не использовать», «Как работает», «Управление клавиатурой», «Анимация», «Правила» требуют вывода из корпуса, а не переизложения снятого, и bulk-проходом не делаются. Пока их нет, запись стоит в статусе `partial`: вёрстка и факты есть, статья — нет. Дописывает шаг R5-09 волны R5.
