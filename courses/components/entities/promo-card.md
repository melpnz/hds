...
**Правило.** Используйте для промокода школы: логотип, плашка кода, условие, кнопка «Открыть код». В продукте 20 узлов — только на странице промокодов.
...
`div.cursor-pointer` без сквозной ссылки: единственный фокусируемый узел — кнопка «Открыть код»; по макету она открывает `PromoCodeModal`, в продукте модалка гидрируется и не снята.

## Управление клавиатурой

Корень (`<div>`) в фокус не попадает. С клавиатуры доступны вложенные записи: `Button` ×20 — у каждой свой раздел.
