...
**Правило.** Рекламный слайд в `AdSlot`: картинки для десктопа и мобильного, метка «РЕКЛАМА», кнопка закрытия. В продукте 52 узла на 4 страницах, все в `Carousel`.
...
Корень (`<div>`) в фокус не попадает. С клавиатуры доступны вложенные записи: `Link` ×4 — у каждой свой раздел.
