...
**Правило.** Независимый двоичный выбор в списке опций. В разметке снятых страниц не встречается, но продуктовый CSS-чанк `similar-courses` несёт его стили: отмеченное и отключённое состояния.

## Когда не использовать

- Для настройки с немедленным эффектом — `Switch`.
- Для взаимоисключающего выбора — `FilterChip` или `SegmentedControl`.
...
В чанке и в story классов перехода у чекбокса нет.
