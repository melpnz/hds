...
У шеврона поля (`svg.absolute.right-3`) — `transition-transform`, 0,15 с с кривой `cubic-bezier(.4,0,.2,1)`. Класса поворота в story нет: список закрыт, и что делает шеврон при открытии, не снято.
