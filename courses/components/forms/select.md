...
Триггер — поле `TextInput` внутри `v-popper`, справа шеврон. Открытый список гидрируется и в снятой разметке отсутствует; поворачивается ли шеврон при открытии, не видно — класса поворота в снятых файлах нет.

## Управление клавиатурой

Корень (`<div>`) в фокус не попадает. На один экземпляр по Tab проходят: 1 поле `input type=text` — так у всех 28. По корпусу (28 узлов): внутри `TextInput` — 28. Фокус внутреннего поля показывает рамка обёртки `focus-within:border-ui-black-850`. 14 из 28 экземпляров на 1440 скрыты (`display:none` у предка) и в порядок табуляции не входят.
