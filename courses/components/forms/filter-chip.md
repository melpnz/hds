...
**Правило.** Используйте для выбора категории, которая сужает выдачу на месте: ряд из «Все» и направлений. В продукте 107 узлов, все внутри `FilterBar` в секциях «Популярные курсы».

## Когда не использовать

- Для перехода на другой адрес — `SegmentedControl`: его сегменты — ссылки с `aria-current`.
