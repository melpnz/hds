...
`div` приходит из SSR с классом `hidden` — так на всех 10 страницах. Открывает его отдельная кнопка с шевроном `arrow-large` рядом с логотипом, за разделителем; сам логотип — ссылка `a[href="/courses"]`. У шеврона объявлен переход `transform` 0,15 с, но класса поворота в снятых файлах нет. Тень `shadow-context-menu-dropdown`, радиус 12.
