...
| **Живая реализация** | [`showcase/components.html#c-header-dropdown`](../../showcase/components.html#c-header-dropdown) |
...
**Правило.** Панель «Все сервисы Хабра» из шапки — одна на страницу, 10/10: четыре проекта с `ProjectIcon`.
...
`div` приходит из SSR с классом `hidden`; открывает его кнопка-логотип в шапке, шеврон которой поворачивается `transition-transform`. Тень `shadow-context-menu-dropdown`, радиус 12.

## Управление клавиатурой

Корень (`<div>`) в фокус не попадает. С клавиатуры доступны вложенные записи: `Link` ×40 — у каждой свой раздел.
...
- **Не снятые состояния.** `open` — разбор в [`components/STATE-CAPTURE.md`](../STATE-CAPTURE.md).
