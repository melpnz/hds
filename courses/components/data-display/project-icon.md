...
- Как самостоятельную ссылку: переход несёт обёртка — ссылка вокруг метки.
- Для сервисов вне Хабра — это `SocialIcon`.

## Как работает

`<span>` 24×24 со скруглением 6 и цветной подложкой. Глиф один на все четыре проекта, различается только цвет подложки: `--color-habr`, `--color-qna`, `--color-career`, `--color-ui-blue-500`.
