...
**Правило.** Используйте для длинного списка ссылок, свёрнутого до трёх строк с раскрытием: направления, города, школы («Популярные направления», «Ещё онлайн-школы»). Счётчик рядом со ссылкой — только в одном списке из пяти, «Популярные направления» на `/courses`. В продукте 5 узлов на 3 страницах.
...
`div` в три колонки (`tablet-only:` две, `phone:` одна) внутри обёртки с `max-h-[94px]` и `overflow-hidden` — по умолчанию список свёрнут.

## Управление клавиатурой

Корень (`<div>`) в фокус не попадает. На один экземпляр по Tab проходят: 16 ссылок — у 1 из 5; 33 ссылки — у 1 из 5; 49 ссылок — у 1 из 5; у остальных 2 — другой состав. По корпусу (129 узлов): `Link` — 129. Свёртка прячет, но не выключает: из 129 ссылок 84 лежат за окном `max-h-[94px]` и всё равно входят в порядок табуляции. При фокусе на такой ссылке браузер сам прокручивает свёрнутый список внутри окна 94px, и ссылка оказывается видна; после этого список так и остаётся сдвинутым.
