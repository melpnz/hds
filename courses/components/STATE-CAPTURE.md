...
## 7. Дополнение 11 сентября 2026: двенадцать figma-only записей

Решением владельца в реестр заведены 12 компонентов макета, которых в
продукте нет (`components/STATES.md` §4, строки с пометкой «добавлена
...
Шесть из двенадцати записей имеют состояния сверх `default` — 17 пар:
...
У шести остальных (`specialization-tag`, `demand-chart`, `loader`,
`feedback-form`, `profession-card`, `vacancy-card`) `requiredStates` —
один `default`.

Итог по реестру после дополнения — **176 пар** «запись × состояние»:
121 сняты источником, 2 дописаны нормативом, 53 не сняты и адресованы
поимённо. Числа §5 описывают 55 записей на момент R1-02 и не
переписываются: прибавка — ровно 29 пар, 12 из них `default`.
