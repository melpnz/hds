...
**Правило.** Используйте для действия без подписи, когда иконка его однозначно называет: стрелки карусели (24 узла в секциях) и закрытие рекламы (8 в `AdSlot`). В продукте 32 узла на 7 страницах.
...
`<button>` с символом спрайта внутри. Левая стрелка у начала карусели получает атрибут `disabled`.

## Управление клавиатурой

Корень — `<button>`: фокус по Tab, действие — Enter или Space. Своего кольца фокуса нет, но нет и сброса: работает фокус браузера. Отключённая стрелка (`disabled`) выпадает из порядка табуляции — так ведёт себя нативная кнопка.
