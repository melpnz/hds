...
for (const [label, target, step] of [
  ["components/manifest.json", manifestPath, "R0-05"],
  ["showcase/components.html", showcasePath, "R0-06"],
]) {
  if (!fs.existsSync(target)) {
    console.error(`Нет файла ${label} — его заводит шаг ${step}. Проверять пока нечего.`);
    process.exit(2);
  }
}
