...
Четыре системы используют отдельные шкалы размеров с общей базой `4px`:
...
Генератор заменяет абсолютные `px`, кратные четырём, в поддерживаемых CSS-свойствах. Селекторы, классы, DOM-контракты и итоговые значения не меняются.
