...
* **Тёмная тема** на момент прохода была проверена только на уровне токенов
  (все 43 разрешаются в обеих темах). Позже дефект disabled-инпута с
  `--header-text` исправлен системной нормализацией на `--background-secondary`.
  Порендерного
