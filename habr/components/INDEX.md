...
* **Dark theme, не проверявшаяся компонент-за-компонентом до этого
  ...
  text-decorated__input` красился в `--header-text` (чистый белый в ОБЕИХ
  темах), поэтому в тёмном интерфейсе поле выглядело светлым пятном. В гайде
  исправлено системной нормализацией на `--background-secondary`, одинаковой
