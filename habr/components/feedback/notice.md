...
| `.tm-notice_info` | **рамка** вместо полосы, padding 16px, нормативный radius 4px (`3px` в production) |

Три разных механики акцента в одном компоненте (полоса / заливка / рамка) —
не унифицированы, каждый вариант со своим набором свойств. Радиус `3px`
нормализован отдельным дизайнерским решением до токена `--habr-radius-4`;
остальные различия не приводить к одному приёму при переносе.
