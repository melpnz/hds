...
* стрелка: на desktop иконка 16 + `padding: 8` → **32×32**; в компактной
  раскладке у стрелки «назад» горизонтальный padding асимметричен, поэтому
  её собственная коробка **24×32**.
...
Чтобы компонент сохранял контракт и в изолированной витрине, у
`.tm-pagination__page` и `.tm-pagination__arrow` явно закреплён
`box-sizing: content-box`. Активная страница остаётся квадратом **32×32**
независимо от внешнего reset, а стрелки сохраняют production-геометрию.
...
| радиус страницы | 3px | `radius` 4px | **принят токен 4px** |
...
Пять оставшихся расхождений **не усреднены и не применены**: в `ui/` остаются
production-значения. Радиус — осознанное исключение: production `3px`
нормализован до системного токена `--habr-radius-4`, потому что это решение
явно принято для гайда. Остальные значения описывают то, что действительно
отрисовано на habr.com. Все пять — на 1–2px либо на один шаг начертания,
