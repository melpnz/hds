...
Размеры интерфейса берутся из `machine/dimension-tokens.json`; CSS-переменные подключаются через `ui/habr.css`. Масштабируемая база — `0.25rem` при эталонном root `16px`; границы и толщина focus ring остаются в `px`, line-height использует безразмерные токены. Новые размеры не записываются литералами. Остальные значения перечислены в `machine/reports/dimension-exceptions.md` и не округляются автоматически.
...
Отдельными адаптивными примерами покрыты article detail, comments, search,
editor, overlay flows, settings и service/error. Публичные article/comments/search
сверены с production; editor, overlays, settings и ошибки подтверждены Figma,
но не заявляются как проверенная production-разметка. Авторизованный
production по-прежнему вне доступного среза.
...
habr/
  machine/
    ...
    catalog.json            компактный маршрутизатор по 90 сущностям
  ...
  examples/generated/       76 изолированных HTML-примеров
...
- Глобальный переключатель темы меняет оболочку viewer и все iframe одновременно; выбор сохраняется при переходах и перезагрузке.
- Точечный фон принадлежит viewer; фон внутри iframe берётся из `--background-primary` активной темы продукта.
...
Статическая проверка сверяет пути, метрики и зафиксированный при миграции baseline: 23 исходные спецификации, 109 production-иконок, 137 editor-иконок и 20 иллюстраций. Браузерная проверка проходит все 90 страниц, проверяет глобальное переключение и сохранение темы, поиск, preview-режимы, ширины, overflow и декодирование ассетов.
