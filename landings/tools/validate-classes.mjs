...
//
// У showcase/lab.css причина держать голые селекторы БЫЛА ровно одна: он
// приносил БАЗУ ДОКУМЕНТА для страницы стенда — сброс полей, фон и шрифт
// body, типографику заголовков, наследование шрифта контролами, поведение
// картинок. Пакет эту базу не давал: ui/foundations.css был пустым каркасом
// до шага R0-03.
//
// ЭТОГО БОЛЬШЕ НЕТ. R0-03 наполнил ui/foundations.css, и вся база документа
// переехала туда: *, body, h1…h6, p, p+p, img, button/input/textarea/select,
// a, [hidden]. Из lab.css она убрана — держать её там значило бы, что стенд
// выглядит правильно, а та же разметка на пустой странице с одним
// ui/landings.css — нет (METHOD §6.1 и §6.2).
//
// Оставшиеся селекторы стенда получили класс оболочки, а не послабление:
//
//   html      → .ml-page       плавная прокрутка и отступ под липкую
//                              панель стенда высотой 110px — мера стенда,
//                              а не значение пакета
//   main      → .ml-shell main поля колонки стенда внутри его каркаса
//   select    → .ml-controls select, .ml-family-tools select — переключатели
//                              продукта, темы и характера принадлежат стенду
//   dialog    → .ml-dialog     окно демонстрационной статьи стенда
//   *         → .ml-page *     остановка переходов стенда по
//                              prefers-reduced-motion
//
// Поэтому список схлопнулся до пустого — как и было записано здесь заранее.
// Пустым он и обязан остаться: любой голый селектор в lab.css теперь роняет
// проверку, потому что это либо молчаливая перекраска продуктовой разметки,
// либо база документа не на своём месте.
