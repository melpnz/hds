 ...
 *
 * Проверка специфична для этого пакета. Причина в источниках: переменных
 * Figma в них нет (get_variable_defs отдаёт по лендингам только logo/*,
 * по бренд-доскам — {} на 4 узлах из 4), и слой токенов выводится из сырых
 * литералов. Как только литерал переехал в переменную, снятое и
