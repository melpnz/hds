...
```json
{
  ...
  "showcase": "showcase/pages.html#p-listing"
}
```

`humanDoc` и `showcase` — куда смотреть человеку. `showcase` обязан указывать
на живой пример с `data-rule` того же `id`. Нет якоря — правило не сдано.
...
## assets.json — изображения и иллюстрации

Описывает не пиксели, а выбор и применение asset: роль в блоке, локальный файл,
соотношение сторон, фон, focal point, responsive crop, alt policy и происхождение.
Для тематических иллюстраций Slerm запись может разрешать генерацию через skill,
но не должна заставлять модель выдумывать другого персонажа.

```json
{
  "id": "hero-course-launch",
  "kind": "illustration",
  "role": "hero-support",
  "file": "ui/assets/illustrations/hero-course-launch.webp",
  "aspectRatio": "1:1",
  "background": "transparent",
  "focalPoint": "50% 45%",
  "responsive": { "mobile": "contain", "desktop": "contain" },
  "altPolicy": "decorative",
  "source": {
    "type": "generated",
    "skill": "image-slerm",
    "references": ["32"],
    "notes": "same mascot identity; topic changes only"
  },
  "humanDoc": "docs/guide/assets.md#hero-course-launch"
}
```

Если `image-slerm` недоступен, потребитель использует существующий `file` или
фиксирует GAP; текстовая имитация стиля не считается заменой референсов.
...
6. изображения выбирать по `assets.json`; Slerm генерировать только skill
   `image-slerm` с локальными оригиналами;
7. чего в пакете нет — не выдумывать: `coverage.boundary` говорит, где граница.
